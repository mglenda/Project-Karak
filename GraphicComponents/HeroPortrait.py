...
class HeroPortrait(Rect):
    ...
    def __init__(self, w: int, h: int, color: tuple, parent: Frame) -> None:
        ...
        self.portrait = Image(self.w,self.h,'_Textures\\Heroes\\Retextured\\Acrobat.png',self)
        self.portrait.set_point(FRAMEPOINT.CENTER,FRAMEPOINT.CENTER)

        self.stats_frame = Rect(self.w,self.h*0.4,(0,0,0),self.portrait)
        self.stats_frame.set_point(FRAMEPOINT.BOTTOM,FRAMEPOINT.BOTTOM)
        self.stats_frame.set_alpha(180)

        self.move_icon = Image(self.h*0.1,self.h*0.1,'_Textures\\HeroPanel\\MovePoints.png',self.portrait)
        self.move_icon.set_point(FRAMEPOINT.BOTTOMLEFT,FRAMEPOINT.CENTER,self.h*0.1,-self.h*0.025,self.stats_frame)

        self.move_mid_icon = NumberImage(self.h*0.08,self.h*0.08,1000,"White",self.portrait)
        self.move_mid_icon.set_point(FRAMEPOINT.TOP,FRAMEPOINT.BOTTOM,0,self.h*0.035,self.move_icon)

        self.move_number = NumberImage(self.h*0.08,self.h*0.08,4,"White",self.portrait)
        self.move_number.set_point(FRAMEPOINT.RIGHT,FRAMEPOINT.LEFT,self.h*0.015,0,self.move_mid_icon)

        self.move_max_number = NumberImage(self.h*0.08,self.h*0.08,4,"White",self.portrait)
        self.move_max_number.set_point(FRAMEPOINT.LEFT,FRAMEPOINT.RIGHT,-self.h*0.015,0,self.move_mid_icon)

        self.health_icon = Image(self.h*0.1,self.h*0.1,'_Textures\\HeroPanel\\HitPoints_Green.png',self.portrait)
        self.health_icon.set_point(FRAMEPOINT.BOTTOMRIGHT,FRAMEPOINT.CENTER,-self.h*0.1,-self.h*0.025,self.stats_frame)

        self.hp_mid_icon = NumberImage(self.h*0.08,self.h*0.08,1000,"Green",self.portrait)
        self.hp_mid_icon.set_point(FRAMEPOINT.TOP,FRAMEPOINT.BOTTOM,0,self.h*0.035,self.health_icon)

        self.hp_number = NumberImage(self.h*0.08,self.h*0.08,5,"Green",self.portrait)
        self.hp_number.set_point(FRAMEPOINT.RIGHT,FRAMEPOINT.LEFT,self.h*0.015,0,self.hp_mid_icon)

        self.hp_max_number = NumberImage(self.h*0.08,self.h*0.08,5,"Green",self.portrait)
