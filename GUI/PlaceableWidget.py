...
class PlaceableWidget(Image):
    ...
    def __init__(self, w: int, h: int, path: str, parent: Frame) -> None:
        ...
        self._power_wheel._set_point(FRAMEPOINT.BOTTOMRIGHT,FRAMEPOINT.BOTTOMRIGHT,x_offset=-w*0.053,y_offset=-h*0.053)

        self._power_text = TextField(parent=self._power_wheel,font_size=30,text='10',font_path=FONT_PATH_NUMBERS)
        self._power_text._set_point(FRAMEPOINT.CENTER,FRAMEPOINT.CENTER)

        self._power_text._resize(w*0.3,h*0.3)
    ...
    def _load_minion(self, minion: Minion):
        self._set_texture(minion.get_path())
        if minion.is_aggresive():
            self._power_text._set_text(str(minion.get_power()))
